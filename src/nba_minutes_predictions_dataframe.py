...
def merge_latest_records(excel_df, combined_df, match_column='Player', date_column='GAME_DATE'):
    """
    Merges data from combined_df into excel_df based on matching names,
    using only the most recent record from combined_df for each name.

    Parameters:
    excel_df: DataFrame from Excel with names
    combined_df: Larger DataFrame with historical data
    match_column: Column name to match between DataFrames (default 'NAME')
    date_column: Column to sort by for most recent record (default 'Game_Date')

    Returns:
    DataFrame: excel_df with additional columns from combined_df's most recent records
    """
    # Ensure the date column is in datetime format
    combined_df[date_column] = pd.to_datetime(combined_df[date_column])

    # Filter combined_df to only include names from excel_df
    filtered_df = combined_df[combined_df[match_column].isin(excel_df[match_column])]

    # Get the most recent record for each name
    latest_records = (filtered_df.sort_values(date_column)
                      .groupby(match_column)
                      .last()
                      .reset_index())

    # Merge the latest records back to the excel_df
    # Using left merge to keep all rows from excel_df
...
def process_excel_data():
    ...
    try:
        ...
        combined_df = combined_df.rename(columns={'PLAYER_NAME' : 'Player'})

        # Merge the data
        result_df = merge_latest_records(excel_df, combined_df)

        # Write results back to Excel
        # Assuming you want to write starting from column A
        #sheet.range('A1').options(index=False).value = result_df
