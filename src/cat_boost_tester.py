...
# Iterate through CatBoost configurations
for depth, learning_rate, l2_leaf_reg, iterations in product(depth_values, learning_rate_values, l2_leaf_reg_values, iterations_values):
    print(f"Testing Configuration: depth={depth}, learning_rate={learning_rate}, l2_leaf_reg={l2_leaf_reg}, iterations={iterations}")
    results = []
    ...
    configuration_results.append(results_df)

# Combine all configurations into a single DataFrame
all_results_df = pd.concat(configuration_results, ignore_index=True)

# Save results to a CSV file
output_file = 'catboost_parameter_results.csv'
all_results_df.to_csv(output_file, index=False)
print(f"Results for all configurations saved to '{output_file}'.")
